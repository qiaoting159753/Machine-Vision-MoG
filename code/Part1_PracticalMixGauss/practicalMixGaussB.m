...
%for each data point
for (cData =1:nData)
    %randomly choose a Gaussian Distribution according to probability
    %distributions[0.3 0.7]
    h = sampleFromDiscrete(mixGauss.weight);
    %draw a sample from the appropriate Gaussian distribution
    %TO DO (d)- replace this
    %The data should 'centered at mean and randomly added a standard deviation range noise'
    data(:,cData) = mixGauss.mean(h) + randn(mixGauss.d,mixGauss.d) .* sqrt(mixGauss.cov(h));                                 
end;
...
for (cIter = 1:nIter)
   ...
   for (cData = 1:nData)
        ...
        %replace this:
        first = mixGaussEst.weight(1) * getGaussProb(data(cData),mixGaussEst.mean(1),mixGaussEst.cov(1));
        second = mixGaussEst.weight(2) * getGaussProb(data(cData),mixGaussEst.mean(2),mixGaussEst.cov(2));
        total = first + second;
        responsibilities(1,cData) = first / total;
        responsibilities(2,cData) = second / total;
   end;
...
for(cData = 1:nData)
    ...
    mu1 = mixGaussEst.mean(1);
    mu2 = mixGaussEst.mean(2);
    sigma1 = mixGaussEst.cov(1);
    sigma2 = mixGaussEst.cov(2);
    
    like1 = exp(-0.5 * ((thisData - mu1)./sigma1).^2) ./ (sqrt(2*pi) .* sigma1);
    like2 = exp(-0.5 * ((thisData - mu2)./sigma2).^2) ./ (sqrt(2*pi) .* sigma2);
    
    like = mixGaussEst.weight(1) * like1 + mixGaussEst.weight(2) * like2;
...
for(cData = 1:nData)
    ...
    %Replace this
    boundValue1 = thisQ(1)*log((mixGaussEst.weight(1) * getGaussProb(thisData, mixGaussEst.mean(1), mixGaussEst.cov(1)))/thisQ(1));
    boundValue2 = thisQ(2)*log((mixGaussEst.weight(2) * getGaussProb(thisData, mixGaussEst.mean(2), mixGaussEst.cov(2)))/thisQ(2));
