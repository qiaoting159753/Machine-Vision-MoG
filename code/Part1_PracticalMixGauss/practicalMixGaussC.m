...
for (cData =1:nData)
    ...
    data(:,cData) = mixGauss.mean(:,h) + chol(mixGauss.cov(:,:,h))*randn(mixGauss.d,1);
end;
...
for (cIter = 1:nIter)
   ...
   for (cData = 1:nData)
        ...
        for i = 1:mixGaussEst.k
            weighted_pr(i) = mixGaussEst.weight(i) * get2DGaussProb(data(:,cData),mixGaussEst.mean(:,i),mixGaussEst.cov(:,:,i));
        end
   ...
   for (cGauss = 1:k) 
        ...
        for index_a = 1:cData
            temp(:,index_a) = data(:,index_a) - mixGaussEst.mean(:,cGauss);
        end
